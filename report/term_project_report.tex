...
The syntax of \textsc{FlowSTLC} is a straightforward extension of STLC with two additional constructs for introducing and eliminating the graded necessity type $\gradedt{\ell}{T}$. We also add records, built-in boolean, natural numbers, and unit type to it:
...
The theorem can be intuitively understood as, if $t$ is a term of type $T$ under a context $\Gamma$, and $\gamma$ is a unary substitution in the interpretation of $\Gamma$. Then we can say that $\gamma\; t$ is in the unary relation at the type $T$.
...
So far we have only specified the typing system of \textsc{FlowSTLC} in a declarative manner, but for actual implementation we need an algorithmic system. In this chapter we introduce the bidirectional typing for \textsc{FlowSTLC}. The typing rules are defined by two mutually recursive functions for \emph{checking} and \emph{synthesis}, of the form:
...
We can swith from synthesis to checking seamlessly:
...
For function abstractions, we check the function body against its return type. The critical check here is that the calculated usage of the bound variable $x$ must $\le$ the declared grade $r$ on the arrow:
...
\begin{proof}
	...
	\begin{itemize}
		...
		Trivial since $\true\in\bbrac{\bool}_\calv$ by definition.
		
		\item \emph{Case} \textsc{T-False}: $\srzero\cdot\Gamma\vdash\false:\bool$.
		
		Trivial since $\false\in\bbrac{\bool}_\calv$ by definition.
...
\begin{proof}
	...
	\begin{itemize}
        ...
        Trivial since $(\true,\true)\in\bbrac{\bool}^{adv}_\calv$ by definition.
        
        \item \emph{Case} \textsc{T-False}: $\srzero\cdot\Gamma\vdash\false:\bool$.
        
        Trivial since $(\false,\false)\in\bbrac{\bool}^{adv}_\calv$ by definition.
        ...
        Trivial since $(0,0)\in\bbrac{\nat}^{adv}_\calv$ by definition.
        
        \item \emph{Case} \textsc{T-Succ}: $\srzero\cdot\Gamma\vdash\suc{n}:\nat$.
        
        Trivial since $(\suc{n},\suc{n})\in\bbrac{\nat}^{adv}_\calv$ by definition.
        
        \item \emph{Case} \textsc{T-Pred}: $\srzero\cdot\Gamma\vdash\prd{n}:\nat$.
        
        Trivial since $(\prd{n},\prd{n})\in\bbrac{\nat}^{adv}_\calv$ by definition.
        
        \item \emph{Case} \textsc{T-IsZero}: $\srzero\cdot\Gamma\vdash\iszero{n}:\bool$.
        
        Trivial since $(\iszero{n},\iszero{n})\in\bbrac{\bool}^{adv}_\calv$ by definition.
        
        \item \emph{Case} \textsc{T-Unit}: $\srzero\cdot\Gamma\vdash\unt:\untt$.
        
        Trivial since $(\unt,\unt)\in\bbrac{\untt}^{adv}_\calv$ by definition.
