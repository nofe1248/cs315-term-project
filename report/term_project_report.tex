...
\begin{figure}[H]
	...
	\begin{aligned}
		...
		\{(\lambda x.t_1,\lambda x.t_2)\mid (\forall v,v'.([v],[v'])\in\llbracket \gradedt{\ell}{T_1}\rrbracket^{adv}_\cale\\
...
\begin{theorem}[Noninterfernce of \textsc{FlowSTLC}]
    For all judgements $x:[A]_r\vdash t:\gradedt{adv}{\bool}$ where $adv\le r$ and $adv\ne r$, then given $\emptyset\vdash v_1:T$ and $\emptyset\vdash v_2:T$ then $[x\mapsto v_1]t\equiv [x\mapsto v_2]t$. Where $\equiv$ is the full $\beta$-equality of terms.
\end{theorem}

For a complete proof for the fundamental theorem and the noninterference theorem, please refer to the appendix.
