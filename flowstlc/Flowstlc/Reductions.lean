...
lemma beta_rename t1 t2 x y : beta_red t1 t2
  ...
  case br_abs s1 s1' T L h f =>
    ...
    intro z hz
    have hz' : z ∉ L ∧ z ≠ x := by
      simpa [Finset.mem_union, Finset.mem_singleton, And.comm] using hz
    have hxz : x ≠ z := by
      intro h'
      exact hz'.2 h'.symm
    simp [subst_open_var s1 ($ y) (lc_var y) x z hxz,
          subst_open_var s1' ($ y) (lc_var y) x z hxz]
    exact (f z hz'.1)
...
lemma beta_red_subst_out t1 t2 x u :
  ...
  case br_abs s1 s2 T L f h =>
    simp only [subst]
    obtain ⟨y, hy⟩ := pick_fresh ([x // u] s1) (L ∪ fv ([x // u] s2) ∪ {x})
    have hy_not_union : y ∉ ((L ∪ fv ([x // u] s2)) ∪ {x}) := by
      intro hy_mem
      exact hy (Finset.mem_union.mpr (Or.inl hy_mem))
    have hy_fv_s1 : y ∉ fv ([x // u] s1) := by
      intro hy_mem
      exact hy (Finset.mem_union.mpr (Or.inr hy_mem))
    have hyL : y ∉ L := by
      intro hy_mem
      exact hy_not_union (Finset.mem_union.mpr (Or.inl (Finset.mem_union.mpr (Or.inl hy_mem))))
    have hy_fv_s2 : y ∉ fv ([x // u] s2) := by
      intro hy_mem
      exact hy_not_union (Finset.mem_union.mpr (Or.inl (Finset.mem_union.mpr (Or.inr hy_mem))))
    have hy_ne : y ≠ x := by
      intro h
      apply hy_not_union
      have hx : y ∈ ({x} : Finset ℕ) := by simp [Finset.mem_singleton, h]
      exact Finset.mem_union.mpr (Or.inr hx)
    have hxy : x ≠ y := by
      intro hxy
      exact hy_ne hxy.symm
    have hβ : beta_red (open₀ ([x // u] s1) ($ y)) (open₀ ([x // u] s2) ($ y)) := by
      simpa [subst_open_var s1 u lcu x y hxy, subst_open_var s2 u lcu x y hxy] using (h y hyL)
...
| para_var : ∀ x, para ($ x) ($ x)
| para_red : ∀ t1 t1' t2 t2' T (L : Finset ℕ),
...
lemma multi_red_abs t1 t2 T (L : Finset ℕ) :
...
lemma multi_red_subst_in t x u1 u2 :
  ...
  case lc_abs u T L h f =>
    ...
    have hy_ne : y ≠ x := by
      intro h_eq
      apply hy
      have hx : y ∈ ({x} : Finset ℕ) := by
        simp [Finset.mem_singleton, h_eq]
      exact Finset.mem_union.mpr (Or.inr hx)
    have hxy : x ≠ y := by
      intro h_eq
      exact hy_ne h_eq.symm
    simpa [subst_open_var u u1 (multi_red_regular _ _ u1mu2).1 x y hxy,
          subst_open_var u u2 (multi_red_regular _ _ u1mu2).2 x y hxy]
      using f y hyL
...
lemma para_to_multi_red : ∀ t t', para t t' → multi_red t t' := by
  ...
  case para_red t1 t1' t2 t2' T L f t2pt2' h h' =>
    ...
    · have  ⟨x, hx⟩ : ∃ x : ℕ, x ∉ (L ∪ (fv t1) ∪ (fv t1')) := by
        exact Infinite.exists_notMem_finset (L ∪ (fv t1) ∪ (fv t1'))
      simp at hx
